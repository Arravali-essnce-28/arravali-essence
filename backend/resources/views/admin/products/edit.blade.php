...
<div class="max-w-5xl mx-auto space-y-8">
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Adjust how this product is organized in the catalog.</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" required
                           ...
                           value="{{ old('name', $product->name) }}" placeholder="Summer Spice Blend">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="number" name="weight" id="weight" step="0.01"
                           class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                           value="{{ old('weight', $product->weight) }}" placeholder="100">
                    @error('weight')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Keep pricing accurate and stock levels healthy.</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="quantity" id="quantity" required min="0"
                           class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                           value="{{ old('quantity', $product->quantity) }}" placeholder="50">
                    @error('quantity')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Maintain polished shots of both front and back views.</p>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="space-y-4">
                    <div class="flex items-center space-x-4">
                        <div class="w-32 h-32 rounded-xl bg-gray-50 border border-dashed border-gray-300 flex items-center justify-center overflow-hidden">
                            @if($product->image)
                                <img src="{{ $product->image }}" alt="{{ $product->name }} front" class="w-full h-full object-cover">
                            @else
                                <span class="text-gray-400 text-sm">No image</span>
                            @endif
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-700">Current front image</p>
                            <p class="text-xs text-gray-500">Replace below to update.</p>
                        </div>
                    </div>

                    <div>
                        <label for="image" class="block text-sm font-medium text-gray-700 mb-3">Replace front image</label>
                        ...
                        <p class="mt-2 text-xs text-gray-500">Leave blank to keep the current photo.</p>
                        @error('image')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="w-32 h-32 rounded-xl bg-gray-50 border border-dashed border-gray-300 flex items-center justify-center overflow-hidden">
                            @if($product->back_image)
                                <img src="{{ $product->back_image }}" alt="{{ $product->name }} back" class="w-full h-full object-cover">
                            @else
                                <span class="text-gray-400 text-sm">No image</span>
                            @endif
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-700">Current back image</p>
                            <p class="text-xs text-gray-500">Upload a rear view for completeness.</p>
                        </div>
                    </div>

                    <div>
                        <label for="back_image" class="block text-sm font-medium text-gray-700 mb-3">Replace back image</label>
                        <input type="file" name="back_image" id="back_image" accept="image/*"
                               class="block w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-600 hover:file:bg-primary-100">
                        <p class="mt-2 text-xs text-gray-500">Optional. Leave blank to keep or omit.</p>
                        @error('back_image')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Keep descriptions informative and enticing.</p>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="short_description" id="short_description"
                           class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                           value="{{ old('short_description', $product->short_description) }}" placeholder="One-line teaser shown across the store">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="5" required
                              class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                              placeholder="Share ingredients, flavor notes, and serving suggestions">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="flex flex-col md:flex-row md:justify-end md:items-center gap-3 md:gap-4 pt-4">
            ...
            <button type="submit" class="inline-flex items-center justify-center px-6 py-2.5 bg-primary-500 text-white font-medium rounded-lg hover:bg-primary-600 focus:outline-none focus:ring-2 focus:ring-primary-400">Update product</button>
        </div>
    </form>
</div>
@endsection